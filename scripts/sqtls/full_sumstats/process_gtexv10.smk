...
rule all:
    input:
        expand("{base_dir}/{type}/gtex_v10/GTEx_Analysis_v10_{type}_all_associations/{type}_gtex_v10_{region}.chr{chrom}_tmp.txt", 
...
rule format_parquet: 
    input:
        "{base_dir}/{type}/gtex_v10/GTEx_Analysis_v10_{type}_all_associations/{region}.v10.cis_sqtl.allpairs.chr{chrom}.parquet"
    output:
