...
rule format_parquet:
    input:
        "{base_dir}/GTEx_Analysis_v10_eQTL_updated/{tissue}.v10.eQTLs.signif_pairs.parquet" 
    params:
        chrom="{chrom}",
    ...
    shell:
        """
        python format_parquet.py --input {input} --output {output} --dbsnp_dir {params.dbsnp_dir} --chrom {params.chrom} --type eQTL
        """
